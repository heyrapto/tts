...
export default function TranslateView() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch("/api/translate", {
        ...
        body: JSON.stringify({ text, targetLanguage: targetLang, voice }),
      });
  ...
  const playHistoryAudio = (item: any) => {
    ...
    // Play if url is active, else regenerate
    if (item.outputUrl) {
  ...
  const regenerateHistoryItem = async (item: any) => {
    setPlayingHistoryId(item.id);
    try {
      const targetLangCode = LANGUAGES.find((l) => l.name === item.language)?.code || "es";
      const res = await fetch("/api/translate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ text: item.input, targetLanguage: targetLangCode, voice: item.voice }),
      });
  ...
  const translateHistory = history.filter((h) => h.type === "translate");

  return (
    <div className="flex-1 flex gap-5 p-6 overflow-hidden h-full">
      {/* Center: Text Input + Translation Output */}
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex-1 flex flex-col border border-gray-200 rounded-xl bg-white shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Source text</span>
            <span className="text-gray-300">→</span>
            <span className="text-xs font-semibold text-violet-600 uppercase tracking-wider">{selectedLang.name}</span>
          </div>
          <textarea
            ...
            placeholder={`Type or paste text to translate... (${characterLimit.toLocaleString()} character limit for your tier)\n\nThe text will be translated and spoken aloud in the target language.`}
            value={text}
        ...
        {/* Translation Result */}
        {translatedText && (
          <div className="mt-4 p-5 bg-gray-50 border border-gray-200 rounded-xl shadow-sm flex flex-col gap-3">
            <div className="flex items-center gap-2 text-[13px] font-semibold text-gray-600">
              <Languages className="w-4 h-4" />
              <span>Translation — {selectedLang.name}</span>
            </div>
            <p className="text-[15px] text-gray-900 leading-relaxed">{translatedText}</p>
          </div>
        )}
      </div>
        ...
        <div className="flex-1 flex flex-col bg-white border border-gray-200 rounded-xl shadow-sm p-5 overflow-y-auto custom-scrollbar">
          ...
          {activeTab === "settings" ? (
            ...
            <div className="flex flex-col flex-1">
              {/* Target Language */}
              <div className="mb-5">
                <span className="text-[13px] font-semibold text-gray-900 block mb-2">Target language</span>
                
                {/* Languages Dropdown with shadcn */}
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="w-[260px] max-h-60 overflow-y-auto custom-scrollbar">
                    ...
                    {LANGUAGES.map((l) => (
                      <DropdownMenuItem
                        ...
                        className={`flex items-center justify-between ${
                          targetLang === l.code ? "bg-gray-100 text-gray-900 font-bold" : ""
                        }`}
                      >
                        <span>{l.name}</span>
                        <span className="text-[10px] text-gray-400 uppercase">{l.code}</span>
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
              ...
              <div className="mb-5">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[13px] font-semibold text-gray-900">Narration voice</span>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent className="w-[260px]">
                    <DropdownMenuLabel>Narration Voices</DropdownMenuLabel>
                    <DropdownMenuSeparator />
                    {VOICES.map((v) => (
                      <DropdownMenuItem
                        key={v.id}
              ...
              {/* Audio Player */}
              {audioUrl && (
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 mt-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <a
                      href={audioUrl}
                      download={`translated-${voice.toLowerCase()}.wav`}
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
              <span className="text-[13px] font-semibold text-gray-900 mb-3 block">Translation History</span>
              {translateHistory.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                  <LanguagesIcon className="w-8 h-8 text-gray-300 mb-2" />
                  <p className="text-xs font-semibold text-gray-400">No translation history yet</p>
                  <p className="text-[10px] text-gray-400 mt-1">Translate scripts to see records here.</p>
                </div>
              ) : (
                <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar">
                  {translateHistory.map((item) => (
                    <div key={item.id} className="p-3 bg-gray-50 border border-gray-200 rounded-xl hover:shadow-sm transition-all duration-150">
                      ...
                      <div className="text-[11.5px] font-medium text-gray-700 leading-snug space-y-1 mb-2.5">
                        <p className="line-clamp-2 text-gray-400 italic font-normal">"{item.input}"</p>
                        <p className="line-clamp-2 text-gray-800">→ {item.outputText}</p>
                      </div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <button
                          onClick={() => {
                            setText(item.input);
                            setTranslatedText(item.outputText || "");
                          }}
                          ...
                          title="Load translation script into editor"
                        >
                        ...
                        {item.outputUrl && (
                          <a
                            href={item.outputUrl}
                            download={`translation-${item.voice?.toLowerCase()}-${item.id}.wav`}
